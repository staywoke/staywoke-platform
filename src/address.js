...
function lazyInit () {
  ...
  DirectionCode = invert(Directional)

  addressMatch = {
    type: flatten(StreetType).sort().filter(function (v, i, arr) { return arr.indexOf(v) === i }).join('|'),
    fraction: '\\d+\\/\\d+',
    state: '\\b(?:' + keys(StateCode).concat(values(StateCode)).map(XRegExp.escape).join('|') + ')\\b',
  ...
  addressMatch.number = '(?<number>\\d+-?\\d*)(?=\\D)'
...
parser.parseAddress = function (address) {
  lazyInit()
  var parts = XRegExp.exec(address, addressMatch.address)
  return parser.normalize_address(parts)
}

parser.parseInformalAddress = function (address) {
  lazyInit()
  var parts = XRegExp.exec(address, addressMatch.informal_address)
  return parser.normalize_address(parts)
}

parser.parsePoAddress = function (address) {
  lazyInit()
  var parts = XRegExp.exec(address, addressMatch.po_address)
  return parser.normalize_address(parts)
}
...
parser.parseIntersection = function (address) {
  lazyInit()
  var parts = XRegExp.exec(address, addressMatch.intersection)
...
export const parseIntersection = parser.parseIntersection
export const parseLocation = parser.parseLocation
export const parseInformalAddress = parser.parseInformalAddress
export const parseAddress = parser.parseAddress
